...
$WinAcmeVersion = "2.2.6.1571" # check GitHub
$WinAcmeArchitecture = "x64"
$WinAcmeDownloadUrl = "https://github.com/win-acme/win-acme/releases/download/v$($WinAcmeVersion)/win-acme.v$($WinAcmeVersion).$($WinAcmeArchitecture).pluggable.zip"
$WinAcmeDir = Join-Path $ToolsDir "win-acme"
$WinAcmeExecutable = Join-Path $WinAcmeDir "wacs.exe"
...
$NginxChocolateyRoot = Split-Path -Parent $NginxPath
$NginxConfFile = Join-Path $NginxChocolateyRoot "conf\nginx.conf"
$NginxSitesDir = Join-Path $DataDir "nginx-sites"
$AcmeChallengeDir = Join-Path $DataDir "acme-challenges"
New-Item -ItemType Directory -Force -Path $CertificatesDir | Out-Null
New-Item -ItemType Directory -Force -Path $AcmeChallengeDir | Out-Null
...
if ($confirmOverwrite -eq 'Y' -or $confirmOverwrite -eq 'y') {
    ...
    $AcmeChallengeDirForNginx = $AcmeChallengeDir.Replace('\', '/')
...
http {
    ...
    # --- Universal server block for HTTP (Port 80) ---
    # This block is set as 'default_server' to catch all requests on port 80
    # that are not explicitly handled by other server_name directives.
    # It integrates both ACME challenge handling and default proxying to Deployer UI.
    server {
        listen 80 default_server; # THIS MUST BE 'default_server' TO CATCH ALL
        server_name _; # Catches requests for any domain

        # ACME Challenge location
        # This must be processed BEFORE other locations for Let's Encrypt to work.
        location /.well-known/acme-challenge/ {
            root   $AcmeChallengeDirForNginx; # Use root, not alias, for this structure
            default_type "text/plain";
            # Important: Do not redirect to HTTPS for ACME challenges.
        }

        # Default proxying to the Deployer UI
        # This location will handle all requests not caught by the ACME challenge location,
        # and not matching a more specific 'server_name' in included configs.
        location / {
    ...
    # These configurations will be managed by the Deployer UI
    # and will take precedence for specific domains over the 'default_server' block.
...
# --- 10. Download and Install win-acme for SSL Certificate Management ---
Write-Host "[10/X] Ensuring win-acme is available for SSL certificate management..." -ForegroundColor Cyan

# Check if win-acme is already installed
if (Test-Path $WinAcmeExecutable) {
    Write-Host "win-acme is already installed at '$WinAcmeExecutable'." -ForegroundColor Green
} else {
    Write-Host "win-acme not found. Downloading and installing to '$WinAcmeDir'..." -ForegroundColor Yellow

    # Ensure the parent directory ($ToolsDir) exists
    New-Item -ItemType Directory -Force -Path $ToolsDir | Out-Null

    $TempZipPath = Join-Path $env:TEMP "win-acme.zip"

    try {
        Write-Host "Downloading from $WinAcmeDownloadUrl..."
        Invoke-WebRequest -Uri $WinAcmeDownloadUrl -OutFile $TempZipPath -UseBasicParsing

        Write-Host "Extracting archive to $WinAcmeDir..."
        Expand-Archive -Path $TempZipPath -DestinationPath $WinAcmeDir -Force

        Remove-Item -Path $TempZipPath -Force

        if (Test-Path $WinAcmeExecutable) {
            Write-Host "win-acme successfully installed." -ForegroundColor Green
        } else {
            throw "Installation failed: wacs.exe not found after extraction. Please check the archive content."
        }

    } catch {
        Write-Host "Error during win-acme installation: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "Please check the download URL and your internet connection. win-acme is not critical for basic Deployer operation, but SSL issuance will fail." -ForegroundColor Yellow
    }
}
